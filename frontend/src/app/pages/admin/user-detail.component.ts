...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, ReactiveFormsModule, MatCardModule, MatFormFieldModule, MatInputModule, MatSlideToggleModule, MatButtonModule],
  template: `
    <div style="max-width:800px;margin:16px auto;padding:0 16px">
      <mat-card>
        ...
        <form [formGroup]="form" (ngSubmit)="save()" style="display:grid;gap:12px">
          ...
          <div>
            <button mat-raised-button color="primary" type="submit" [disabled]="form.invalid || busy">Salvar</button>
          </div>
        </form>
      </mat-card>
      <mat-card style="margin-top:16px">
        <h2>Reset de senha</h2>
        <form [formGroup]="formPwd" (ngSubmit)="resetPwd()" style="display:grid;gap:12px">
          ...
          <button mat-raised-button color="warn" type="submit" [disabled]="formPwd.invalid || busy">Redefinir senha</button>
        </form>
...
export class AdminUserDetailComponent {
  ...
  formPwd = this.fb.group({ new_password: ['', [Validators.required, Validators.minLength(8)]] });

  ngOnInit(){
    this.id = Number(this.route.snapshot.paramMap.get('id'))||0;
    if(!this.id) return;
  this.api.getUser(this.id).subscribe({ next: (u:any)=>{ this.form.patchValue({ login: u.login as string, nome: u.nome as string, email: u.email as string, is_active: !!u.is_active }); }, error: ()=>{} });
  }

  save(){
    if(this.form.invalid) return;
    this.busy = true; this.msg=''; this.err='';
    const { nome, email, is_active } = this.form.getRawValue();
    this.api.patchUser(this.id, { nome: nome||'', email: email||'', is_active: !!is_active }).subscribe({ next: ()=>{ this.msg='Salvo'; this.busy=false; }, error: ()=>{ this.err='Falha ao salvar'; this.busy=false; } });
  }
  ...
  resetPwd(){
    ...
    this.api.resetPassword(this.id, pw).subscribe({ next: ()=>{ this.msg='Senha redefinida'; this.busy=false; }, error: ()=>{ this.err='Falha ao redefinir senha'; this.busy=false; } });
  }
}
