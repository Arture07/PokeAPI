...
@Component({
  ...
  template: `
    <div
      style="max-width:800px;margin:16px auto;padding:0 16px;display:grid;gap:16px"
    >
      <mat-card>
        <h1>Minha Conta</h1>
        <form
...
export class AccountSettingsComponent {
  ...
  formPwd = this.fb.group({
    current_password: this.fb.control<string>('', {
      validators: [Validators.required],
    }),
    new_password: this.fb.control<string>('', {
  ...
  saveProfile() {
    ...
    this.auth.updateMe({ nome: nome || '', email: email || '' }).subscribe({
      next: () => {
        this.msg = 'Dados atualizados';
        this.busy = false;
      },
      error: () => {
        this.err = 'Falha ao atualizar';
        this.busy = false;
      },
    });
  }
  ...
  changePwd() {
    ...
    const { current_password, new_password, confirm } =
      this.formPwd.getRawValue();
    if (new_password !== confirm) {
      this.errPwd = 'Senhas não conferem';
      return;
    }
    this.busy = true;
    this.msgPwd = '';
    ...
    this.auth.changePassword(current_password!, new_password!).subscribe({
      next: () => {
        this.msgPwd = 'Senha alterada';
        this.busy = false;
      },
      error: () => {
        this.errPwd = 'Falha ao alterar senha';
        this.busy = false;
      },
    });
  }
